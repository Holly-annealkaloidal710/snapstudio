...
export default function DashboardPage() {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (!currentProjectId) {
        const projectName = `${productName} - ${new Date().toLocaleDateString('vi-VN')}`;
  ...
  const handleDownloadAll = async () => {
    try {
      ...
      toast.success("Tải tất cả ảnh thành công!");
    } catch (error) {
      toast.error("Lỗi khi tải ảnh");
    }
  };
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      <DashboardHeader
        userName={profile.full_name || profile.email?.split('@')[0] || 'Bạn'}
        projectId={projectId}
      ...
      {!projectId && generatedImages.length === 0 && !soloResult && (
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div>
              <h3 className="text-xl font-bold text-gray-900 mb-4">Thống kê của bạn</h3>
              <DashboardStats />
            </div>
            <div>
              <RecentProjects />
          ...
          <div>
            <h3 className="text-xl font-bold text-gray-900 mb-4">Ví dụ ảnh được tạo bởi SnapStudio</h3>
            <CompactSampleGrid />
          </div>
        </div>
      )}
