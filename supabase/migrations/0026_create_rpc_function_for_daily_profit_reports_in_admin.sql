...
BEGIN
  ...
  SELECT
    ...
    COUNT(r.images_generated) AS images,
    SUM(r.cost_vnd) AS cost_vnd,
    COALESCE(SUM(o.amount), 0) AS revenue_vnd,  -- Join with orders for revenue
    COALESCE(SUM(o.amount), 0) - SUM(r.cost_vnd) AS profit_vnd,
    CASE 
      WHEN COUNT(r.images_generated) > 0 THEN 
        SUM(r.cost_vnd)::DECIMAL / COUNT(r.images_generated)
      ELSE 0 
